...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/*">
		...
		<xsl:variable name="cacShipmentStageDriverPerson" select="cac:Shipment/cac:ShipmentStage/cac:DriverPerson"/>
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			<xsl:with-param name="errorCodeValidate" select="'4185'"/>
			...
			<xsl:for-each select="$cacAdditionalDocumentReference">
				...
				<xsl:if test="./cbc:DocumentTypeCode[text() = '01']">
					<xsl:call-template name="existAndRegexpValidateElement">
						...
						<xsl:with-param name="node" select="./cbc:ID"/>
						<xsl:with-param name="regexp" select="'^[0-9]{4}-[0-9]{2}-[0-9]{3}-[0-9]{4}$'"/>
					</xsl:call-template>		 
				</xsl:if>
		...
		<xsl:call-template name="existAndRegexpValidateElement">
			...
			<xsl:with-param name="regexp" select="'^[0-9]{1,15}$'"/>
		</xsl:call-template>	
		...
		<xsl:if test="cac:SellerSupplierParty">
			...
			<xsl:if test="$cbcSellerSupplierPartyAccountID = $cbcDespatchSupplierAccountID or $cbcSellerSupplierPartyAccountID = $cbcDeliveryCustomerAccountID"> 
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'4053'" />
					<xsl:with-param name="warningMessage" select="'Error Expr Regular guia remision (codigo: 4053)'" />
				</xsl:call-template>
			</xsl:if>
		...
		<xsl:call-template name="existAndRegexpValidateElement">
			...
			<xsl:with-param name="regexp" select="'^(01|1|02|2|04|4|08|8|09|9|13|17|18|19)$'"/>
		</xsl:call-template>
		...
		<xsl:if test="$cbcShipmentHandlingCode[text() != '08' and text() != '8']">
			...
			<xsl:if test="$cbcShipmentTotalTransportHandlingUnitQuantity">
				<xsl:call-template name="addWarning">
					...
					<xsl:with-param name="warningMessage" select="'Error Expr Regular guia remision (codigo: XXXX)'" />
				</xsl:call-template>
			</xsl:if>
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<xsl:with-param name="regexp" select="'(?!(^0+(\.0+)?$))(^\d{1,12}(\.\d{1,3})?$)'"/>
			<xsl:with-param name="isError" select="false()"/>
		</xsl:call-template>			
		
		<xsl:if test="$cbcShipmentGrossWeightMeasure">
			<xsl:call-template name="existAndRegexpValidateElement">
				<xsl:with-param name="errorCodeNotExist" select="'4194'"/>
		...
		<!-- cacShipmentStageCarrierParty
		cacShipmentStageDriverPerson
		cacTransportMeansRoadTransport  -->
			...
			<xsl:if test="not($cacTransportMeansRoadTransport/cbc:LicensePlateID)">
                <xsl:call-template name="rejectCall">
                	<xsl:with-param name="errorCode" select="'1067'"/> 
           			<xsl:with-param name="errorMessage" select="'Error Expr Regular guia remision (codigo: 1066)'"/>
           		</xsl:call-template>
            </xsl:if>
