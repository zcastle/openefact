...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test='not(regexp:match(./cac:AccountingSupplierParty/cac:Party/cac:PartyLegalEntity/cbc:RegistrationName,"^(.{1,100})$"))'>
    ...
    <xsl:if test="(date:seconds(date:difference(concat($fechaRangos,'-00:00'),$issuedate)) &lt; 0)">
      <xsl:call-template name="rejectCall"> <xsl:with-param name="errorCode" select="4036" /> <xsl:with-param name="errorMessage" select="'4036 Error resumen diario de reversiones'" /> </xsl:call-template>
    </xsl:if>
    ...
    <xsl:for-each select="sac:VoidedDocumentsLine">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
		  ...
		  <xsl:if test="not(./cbc:DocumentTypeCode = '20' or ./cbc:DocumentTypeCode = '40')">
...
<!--           <xsl:if test="./cbc:DocumentTypeCode='20' and not(substring(./sac:DocumentSerialID,1,1)='R')">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
			<xsl:if test='not(regexp:match(./sac:VoidReasonDescription,"^(.{1,100})$"))'>
			  <xsl:call-template name="rejectCall"> <xsl:with-param name="errorCode" select="2314" /> <xsl:with-param name="errorMessage" select="'2314 Error resumen diario de reversiones'" /> </xsl:call-template>
			</xsl:if>
		</xsl:otherwise>
      </xsl:choose>
    </xsl:for-each>
