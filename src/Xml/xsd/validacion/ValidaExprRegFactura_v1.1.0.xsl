...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<!-- INI wsandovalh se agrega xsl de validaciones -->
	<xsl:include href="validate_utils.xsl" dp:ignore-multiple="yes" />
	<!-- FIN wsandovalh se agrega xsl de validaciones -->
    ...
    <xsl:template match="/*">
        ...
        <!--  IMR 2016-08-26 Factura Guia Remitente o Transportista-->
        <xsl:variable name="SUNATEmbededDespatchAdvice" select="$sacAdditionalInformation/sac:SUNATEmbededDespatchAdvice"/>
        ...
        <xsl:if test="$sacSUNATTransactionID">
            ...
            <!-- De existir el elemento, validar que el campo ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:SUNATTransaction/cbc:ID
				...
				OBS-4042. -->
			<!--  IMR 2016-08-26 Factura Guia Remitente 10 o Transportista 11 -->
            <!--  <xsl:if test="not($sacSUNATTransactionID[text() = '01' or text() = '02' or text() = '03' or text() = '04' or text() = '05' or text() = '06'])"> -->
            <xsl:if test="not($sacSUNATTransactionID[text() = '01' or text() = '02' or text() = '03' or text() = '04' or text() = '05' or text() = '10' or text() = '11'])">
            <!--  Fin IMR 2016-08-26 -->
            ...
            <!-- IMR - RF07 traslado de bienes, sacSUNATTransactionID igual a 10 o igual a 11
					y no existe el tag “SUNATEmbededDespatchAdvice”, generar observación 4120 – El XML no existe o no existe el tag SUNATEmbededDespatchAdvice de Información de sustento de traslado.-->
      		<xsl:if test="$sacSUNATTransactionID[text() = '10' or text() = '11']">
	            <xsl:if test="not($SUNATEmbededDespatchAdvice)">
	                <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4120'"/>
                		<xsl:with-param name="warningMessage" select="' Esta indicando que se trata de una factura guia pero no se consigna informacion que sustenta el traslado.'"/>
                    </xsl:call-template>
	            </xsl:if>
	        </xsl:if>
            <!-- Fin IMR 2016-07-07 -->

            <!-- IMR 2016-07-07- RF07 traslado de bienes, sacSUNATTransactionID diferente a 10 y diferente a 11
					y existe el tag “SUNATEmbededDespatchAdvice”, generar observación 4121 – Para el tipo de operación no se consigna el tag SUNATEmbededDespatchAdvice de Información de sustento de traslado.-->
	        <xsl:if test="$sacSUNATTransactionID[text() != '10' and text() != '11']">
	            <xsl:if test="$SUNATEmbededDespatchAdvice">
	                <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4121'"/>
                		<xsl:with-param name="warningMessage" select="' Esta ingresando informacion de traslado de bienes en una transaccion que no es factura guia.'"/>
                    </xsl:call-template>
	            </xsl:if>
	        </xsl:if>
            <!-- FIn IMR 2016-07-07 -->
		...
		<!-- IMR 2016-07-07 - De existir el tag de información que sustenta el traslado.-->
		<xsl:if test="$SUNATEmbededDespatchAdvice">
			<!-- IMR - RF07 traslado de bienes, existe el tag SUNATEmbededDespatchAdvice
			de no existir “sac:SUNATTransaction/cbc:ID” muestra observación 4047 - Indicar si corresponde a Factura Electrónica Remitente o Factura Electrónico Transportista.-->
			<xsl:if test="not($sacSUNATTransactionID)">
                <xsl:call-template name="addWarning">
	                <xsl:with-param name="warningCode" select="'4047'"/>
                	<xsl:with-param name="warningMessage" select="' Esta ingresando informacion de traslado de bienes en una transaccion que no indica tipo de operacion.'"/>
                 </xsl:call-template>
            </xsl:if>

            <!-- Debe existir el código de Leyenda = 2008
                   de no existir muestra observación 4122 - Factura con información que sustenta el traslado, debe registrar mensaje 2008.   -->
			<xsl:if test="string(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:AdditionalProperty/cbc:ID)!='2008'">
				<xsl:call-template name="addWarning">
	                <xsl:with-param name="warningCode" select="'4122'"/>
                	<xsl:with-param name="warningMessage" select="' Debe consignar la Leyenda 2008 (Este documento sustenta el traslado de bienes)  .'"/>
                 </xsl:call-template>
			</xsl:if>
		</xsl:if>
		<!-- Fin IMR 2016-07-07 -->
	   ...
	   <!-- Firma Digital de Documento -->
        ...
        <xsl:for-each select="$sacAdditionalInformation/sac:SUNATEmbededDespatchAdvice">
            <xsl:if test="$sacSUNATTransactionID[text() = '06']">
        -->
        <!--  Fin IMR 2016-08-26 Factura Guia Remitente o Transportista -->
                <!-- a.1 + a.2 -->
        <!-- IMR 2016-08-26 Factura Guia Remitente o Transportista -->
        <!--
        ...
        <xsl:for-each select="$SUNATEmbededDespatchAdvice">

            <!-- Validar la existencia o no existencia de tags en Factura electrónica remitente. -->

            <xsl:if test="$sacSUNATTransactionID[text() = '10']">
                 <!-- No Debe de existir el tag Documento de Referencia.
					De no cumplir generar observación 4123 - sac:SUNATEmbededDespatchAdvice - Para Factura Electrónica Remitente no debe consignar datos en documento de referencia (cac:OrderReference). -->
                <xsl:if	test="./sac:OrderReference">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4123'"/>
                		<xsl:with-param name="warningMessage" select="' Documentos de referencia no se consigna en una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
				</xsl:if>

				<!-- Debe de existir el tag Sujeto que realiza la confirmación.
					De no cumplir generar observación 4124 - cac:Shipment - Debe indicar sujeto que realiza el traslado de bienes (1: Vendendor o 2: Comprador). -->
                <xsl:if	test="not(./sac:Shipment/cbc:HandlingCode)">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4124'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar sujeto que realiza el traslado de bienes para una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
				</xsl:if>

				<!-- Debe de existir el tag de modalidad de transporte.
					De no cumplir generar código de error 4125 - cac:Shipment - Debe indicar  modalidad de transporte para el sustento de traslado de bienes (cbc:TransportModeCode) -->
                <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode)">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4125'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar modalidad de transporte para una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
                </xsl:if>

                <!-- Debe de existir el tag de Fecha de inicio de traslado o fecha de entrega de bienes al transportista.
					De no cumplir generar código de error 4126 - cac:Shipment - Debe indicar  fecha de inicio de traslado para el sustento de traslado de bienes (cac:TransitPeriod/cbc:StartDate). -->
                <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:TransitPeriod/cbc:StartDate)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4126'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar fecha de inicio de traslado para una transaccion de factura guia.'"/>
                    </xsl:call-template>
                </xsl:if>


				<!-- Debe de existir el tag de Dirección del punto de llegada.
					De no cumplir generar código de error 4127 - cac:Shipment - Para Factura Electrónica Remitente Debe indicar el punto de partida para el sustento de traslado de bienes (cac:DeliveryAddress). -->
                <xsl:if test="not(./sac:Shipment/cac:Delivery/cac:DeliveryAddress)">
                	<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4127'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar punto de llegada para una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
                </xsl:if>

				<!-- Debe de existir el tag de Dirección del punto de partida.
					De no cumplir generar código de error 4128 - cac:Shipment - Para Factura Electrónica Remitente Debe indicar el punto de partida para el sustento de traslado de bienes (cac:OriginAddress). -->
                <xsl:if test="not(./sac:Shipment/cac:OriginAddress)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4128'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar punto de partida para una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
                </xsl:if>


                <!-- No Debe de existir el tag Indicador subcontratación.
					De no cumplir generar código de error 4129 - sac:SUNATEmbededDespatchAdvice - Para Factura Electrónica Remitente no se consigna indicador de subcontratación (cbc:MarkAttentionIndicator). -->
                <xsl:if test="./cbc:MarkAttentionIndicator">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4129'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar indicador de subcontratacion para una transaccion de factura guia remitente.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test="./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode">
	                <!-- El tag modalidad de transporte (cbc: TransportModeCode) debe ser = 01 o 02,
	                	de no cumplir generar observación 4136 S– 'cbc:TransportModeCode -  dato ingresado en modalidad de transporte no es valido (Catálogo Nro. 18). -->
		            <xsl:if test="./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode[text() != '01' and text() != '02']">
		                <xsl:call-template name="addWarning">
		                	<xsl:with-param name="warningCode" select="'4043'"/>
		           			<xsl:with-param name="warningMessage" select="' Ingrese una Modalidad de transporte valido (Catalogo Nro. 18).'"/>
		           		</xsl:call-template>
		            </xsl:if>

		            <xsl:if test="./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode[text() = '01']">
		            	<xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:CarrierParty)">
			                <xsl:call-template name="addWarning">
			                	<xsl:with-param name="warningCode" select="'2534'"/>
			           			<xsl:with-param name="warningMessage" select="' Ingrese datos del transportista para una operacion de factura guia remitente de transporte publico'"/>
			           		</xsl:call-template>
			            </xsl:if>

		            	<xsl:if test="./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID or ./sac:Shipment/sac:ShipmentStage/sac:DriverPerson ">

			                <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID) or not(./sac:Shipment/sac:ShipmentStage/sac:DriverPerson)">
				                <xsl:call-template name="rejectCall">
				                	<xsl:with-param name="errorCode" select="'4156'"/>
				           			<xsl:with-param name="errorMessage" select="' Para factura guia remitente con transporte publico de ingresar informacion requerida al transportista debe consignarse la información completa (vehiculos y conductores).'"/>
				           		</xsl:call-template>
				        	</xsl:if>
			            </xsl:if>
		            ...
		            <xsl:if test="./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode[text() = '02']">

		            	<xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID)">
			                <xsl:call-template name="addWarning">
			                	<xsl:with-param name="warningCode" select="'4157'"/>
			           			<xsl:with-param name="warningMessage" select="' Ingrese vehiculo para una operacion de factura guia remitente de transporte publico'"/>
			           		</xsl:call-template>
			            </xsl:if>

			            <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/sac:DriverPerson)">
			                <xsl:call-template name="addWarning">
			                	<xsl:with-param name="warningCode" select="'4158'"/>
			           			<xsl:with-param name="warningMessage" select="' Ingrese conductores para una operacion de factura guia remitente de transporte publico'"/>
			           		</xsl:call-template>
			            </xsl:if>

		            	<xsl:if test="./sac:Shipment/sac:ShipmentStage/cac:CarrierParty">
			                <xsl:call-template name="addWarning">
			                	<xsl:with-param name="warningCode" select="'4159'"/>
			           			<xsl:with-param name="warningMessage" select="' Para Factura guia remitente de transporte privado no se consigna datos del transportista'"/>
			           		</xsl:call-template>
			            </xsl:if>
		            </xsl:if>
		        </xsl:if>

		        <xsl:if test="./sac:Shipment/cbc:HandlingCode and ./sac:Shipment/cbc:HandlingCode[text() != '1' and text()!= '2']">
	                <xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4153'"/>
	           			<xsl:with-param name="warningMessage" select="'Sujeto que realiza el traslado no es valido (consigne 1 para Emisor Electrónico – Vendedor o 2 Comprador – Adquiriente o poseedor del bien).'"/>
	           		</xsl:call-template>
	            </xsl:if>

	            <xsl:if test="./sac:Shipment/cbc:GrossWeightMeasure/@unitCode and ./sac:Shipment/cbc:GrossWeightMeasure/@unitCode != 'KGM'">
	                <xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4154'"/>
	           			<xsl:with-param name="warningMessage" select="'Peso bruto total de la factura debe expresarse en Kilogramos (KGM estandar internacional).'"/>
	           		</xsl:call-template>
	            </xsl:if>

	            <xsl:if test='./sac:Shipment/cbc:GrossWeightMeasure and not(regexp:match(./sac:Shipment/cbc:GrossWeightMeasure,"^[0-9]{1,12}(\.[0-9]{1,2})?$"))'>
	                <xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4155'"/>
	           			<xsl:with-param name="warningMessage" select="'Peso bruto total de la factura no tiene un formato no valido decimal (12,2).'"/>
	           		</xsl:call-template>
	            </xsl:if>

            </xsl:if>

            <!-- Validar la existencia o no existencia de tags en Factura electrónica transportista. -->
            <xsl:if test="$sacSUNATTransactionID[text() = '11']">

                <!-- Debe de existir el tag Documento de Referencia.
					De no cumplir generar observación 4130  -->
                <xsl:if	test="not(./sac:OrderReference)">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4130'"/>
                		<xsl:with-param name="warningMessage" select="' Indicar documento de referencia para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
				</xsl:if>

				<!-- No debe existir el tag Datos del destinatario.
					De no cumplir generar observación 4131  -->
                <xsl:if	test="./cac:DeliveryCustomerParty">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4131'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar datos del destinatario para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
				</xsl:if>

				<!-- No Debe de existir el tag Sujeto que realiza el traslado.
					De no cumplir generar observación 4132  -->
                <xsl:if	test="./sac:Shipment/cbc:HandlingCode">
					<xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4132'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar Sujeto que realiza el traslado para una transaccion de factura guia transportista'"/>
                    </xsl:call-template>
				</xsl:if>

				<!-- No Debe de existir el peso bruto total de la factura.
					De no cumplir generar código de error 4133 -->
                <xsl:if test="./sac:Shipment/cbc:GrossWeightMeasure">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4133'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar peso bruto de la factura para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
                </xsl:if>

				<!-- No Debe de existir el tag de modalidad de transporte.
					De no cumplir generar código de error 4134  -->
                <xsl:if test="./sac:Shipment/sac:ShipmentStage/cbc:TransportModeCode">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4134'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar modalidad de transporte para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
                </xsl:if>


                <!-- Debe de existir el tag de Fecha de inicio de traslado o fecha de entrega de bienes al transportista.
					De no cumplir generar código de error 4126 -->
                <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:TransitPeriod/cbc:StartDate)">
                    <xsl:call-template name="rejectCall">
	                    <xsl:with-param name="errorCode" select="'4126'"/>
                		<xsl:with-param name="errorMessage" select="' Indicar fecha de inicio de traslado para una transaccion de factura guia.'"/>
                    </xsl:call-template>
                </xsl:if>
                ...
                <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID)">
	                <xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4157'"/>
	           			<xsl:with-param name="warningMessage" select="' Ingrese vehiculo para una operacion de factura guia transportista.'"/>
	           		</xsl:call-template>
	            </xsl:if>
	            ...
	            <xsl:if test="not(./sac:Shipment/sac:ShipmentStage/sac:DriverPerson)">
	                <xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4158'"/>
	           			<xsl:with-param name="warningMessage" select="' Ingrese conductores para una operacion de factura guia transportista.'"/>
	           		</xsl:call-template>
	            </xsl:if>

                <!-- No Debe de existir el tag de Dirección del punto de llegada.
					De no cumplir generar código de error 4135 -->
                <xsl:if test="./sac:Shipment/cac:Delivery/cac:DeliveryAddress">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4135'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar punto de llegada para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
                </xsl:if>

				<!-- No Debe de existir el tag de Dirección del punto de partida.
					De no cumplir generar código de error 4136 -->
                <xsl:if test="./sac:Shipment/cac:OriginAddress">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4136'"/>
                		<xsl:with-param name="warningMessage" select="' No debe consignar punto de partida para una transaccion de factura guia transportista.'"/>
                    </xsl:call-template>
                </xsl:if>

            </xsl:if>

            <!-- Si existe datos de referencia validar. -->
            <xsl:for-each select="$SUNATEmbededDespatchAdvice/sac:OrderReference">
	            <!-- Se incluyo bloque en un For, como en Validador Java - Inicio -->

	            <!-- Que existan el tag número de documento
					de no existir marcar código de error 4137-->
	            <xsl:if test="not(./cbc:ID) or not(./cbc:ID[text() = ''])">
	                <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4137'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese el numero del comprobante que referencia el traslado de bienes.'"/>
                    </xsl:call-template>
	            </xsl:if>

	            <!-- Validar que exista el tag tipo de documento que inicia el traslado de bienes
					de no existir generar código de observación 4138 -->
	            <xsl:if test="not(./sac:OrderTypeCode)">
	                <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4138'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese el tipo de comprobante que referencia el traslado de bienes.'"/>
                    </xsl:call-template>
	            </xsl:if>

	            <xsl:if test="./sac:OrderTypeCode">

		            <xsl:if test="./sac:OrderTypeCode[text() != '01' and text() != '09']">
		                <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4139'"/>
	                		<xsl:with-param name="warningMessage" select="' Tipo de comprobante que referencia el traslado de bienes es un tipo de comprobante no valido (01 - Factura o 09 - Guia de Remision). '"/>
                    	</xsl:call-template>
		            </xsl:if>

		            <xsl:if test="./sac:OrderTypeCode[text() = '01'] and not(regexp:match(./cbc:ID,'^[F][A-Z0-9]{3}-[0-9]{1,8}$|^(E001)-[0-9]{1,8}$|^[0-9]{1,4}-[0-9]{1,8}$'))">
	                    <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4140'"/>
	                		<xsl:with-param name="warningMessage" select="' Serie y numero de comprobante que referencia el traslado de bienes no cumple con un formato valido para factura. '"/>
                    	</xsl:call-template>
	                </xsl:if>

	                <xsl:if test="./sac:OrderTypeCode[text() = '09'] and not(regexp:match(./cbc:ID,'^[T][A-Z0-9]{3}-[0-9]{1,8}$|^(EG01)-[0-9]{1,8}$|^[0-9]{1,4}-[0-9]{1,8}$'))">
	                    <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4140'"/>
	                		<xsl:with-param name="warningMessage" select="' Serie y numero de comprobante que referencia el traslado de bienes no cumple con un formato valido para guia de remision. '"/>
                    	</xsl:call-template>
	                </xsl:if>
	            </xsl:if>

	            <xsl:if test="not(./sac:DocumentReference/cac:IssuerParty/cac:PartyIdentification/cbc:ID)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4141'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese RUc del emisor del comprobante que referencia el traslado de bienes. '"/>
                   	</xsl:call-template>
				</xsl:if>

				<xsl:if test="./sac:DocumentReference/cac:IssuerParty/cac:PartyIdentification/cbc:ID and not(regexp:match(./sac:DocumentReference/cac:IssuerParty/cac:PartyIdentification/cbc:ID,'^[0-9]{11}$'))">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4142'"/>
                		<xsl:with-param name="warningMessage" select="' RUC del emisor del comprobante que referencia el traslado de bienes no cumple con un formato valido. '"/>
                   	</xsl:call-template>
				</xsl:if>

				<xsl:if test="./sac:DocumentReference/cac:IssuerParty/cac:PartyName/cbc:Name and not(regexp:match(./sac:DocumentReference/cac:IssuerParty/cac:PartyName/cbc:Name,'^(?!\s*$).{3,100}$'))">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4146'"/>
                		<xsl:with-param name="warningMessage" select="' Nombres y Apellidos, o Razon Social del Emisor del documento que referencia el traslado de bienes no cumple con un formato valido. '"/>
                   	</xsl:call-template>
				</xsl:if>

	        </xsl:for-each>

            <!-- Si existe datos de destinatario validar. -->
            <xsl:for-each select="./cac:DeliveryCustomerParty">

            	<!-- Debe existir el tag de numero de documento de identidad del destinatario.
					De no cumplir generar código de error 4147 -->
                <xsl:if test="not(./cbc:CustomerAssignedAccountID)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4147'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese el numero de documento de identidad del destinatario de la informacion que sustento de traslado.'"/>
                    </xsl:call-template>
                </xsl:if>

            	<xsl:if test="./cbc:CustomerAssignedAccountID">

                    <xsl:if test="not(./cbc:CustomerAssignedAccountID/@schemeID)">
	                    <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4148'"/>
	                		<xsl:with-param name="warningMessage" select="' Para informacion que sustenta el traslado de bienes, debe Ingresar el tipo de documento de identidad del destinatario.'"/>
	                    </xsl:call-template>
	                </xsl:if>

	                <xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID">

		                <xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID != '0' and ./cbc:CustomerAssignedAccountID/@schemeID != '1' and ./cbc:CustomerAssignedAccountID/@schemeID != '4' and ./cbc:CustomerAssignedAccountID/@schemeID != '6' and ./cbc:CustomerAssignedAccountID/@schemeID != '7' and ./cbc:CustomerAssignedAccountID/@schemeID != 'A'">
		                    <xsl:call-template name="addWarning">
			                    <xsl:with-param name="warningCode" select="'4149'"/>
		                		<xsl:with-param name="warningMessage" select="' el tipo de documento de identidad del destinatario no es valido (catalogo N° 06).'"/>
		                    </xsl:call-template>
		                </xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = '0' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[A-Za-z0-9]{1,15}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="errorCode" select="'4150'"/>
	                			<xsl:with-param name="errorMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato valido para el tipo ingresado. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = '1' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[0-9]{8}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4150'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato valido para DNI. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = '4' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[A-Za-z0-9]{1,12}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4150'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato para Carnet de Extranjeria. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = '6' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[0-9]{11}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4150'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato para RUC. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = '7' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[A-Za-z0-9]{1,12}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4150'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato para PASAPORTE. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:CustomerAssignedAccountID/@schemeID = 'A' and not(regexp:match(./cbc:CustomerAssignedAccountID,'^[A-Za-z0-9]{1,15}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4150'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del destinatario no cumple con un formato para el tipo ingresado. '"/>
                    		</xsl:call-template>
	                	</xsl:if>
                ...
                <xsl:if test="not(./cac:Party/cac:PartyLegalEntity/cbc:RegistrationName)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4151'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese nombres y apeelidos, o razon social del destinatario de la informacion que sustento de traslado.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test=' ./cac:Party/cac:PartyLegalEntity/cbc:RegistrationName and not(regexp:match(./cac:Party/cac:PartyLegalEntity/cbc:RegistrationName,"^(?\s*$).{3,100}$"))'>
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4152'"/>
                		<xsl:with-param name="warningMessage" select="' Nombres y apeelidos, o razon social del destinatario no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>
	        ...
	        <!-- Si existe datos de transportista validar. -->
            <xsl:for-each select="sac:Shipment/sac:ShipmentStage/cac:CarrierParty">
                ...
                <xsl:if test="not(./cac:PartyIdentification/cbc:ID)">
                    <xsl:call-template name="addWarning">
                		...
                		<xsl:with-param name="warningMessage" select="' Ingrese el numero de documento de identidad del transportista de la informacion que sustento de traslado.'"/>
                    </xsl:call-template>
                </xsl:if>

            	<xsl:if test="./cac:PartyIdentification/cbc:ID">

                    <xsl:if test="not(./cac:PartyIdentification/cbc:ID/@schemeID)">
	                    <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4161'"/>
	                		<xsl:with-param name="warningMessage" select="' Ingrese el tipo de documento de identidad del transportista de la informacion que sustenta el traslado de bienes.'"/>
	                    </xsl:call-template>
	                </xsl:if>

	                <xsl:if test="./cac:PartyIdentification/cbc:ID/@schemeID">

		                <xsl:if test="./cac:PartyIdentification/cbc:ID/@schemeID != '6'">
		                    <xsl:call-template name="addWarning">
			                    <xsl:with-param name="warningCode" select="'4162'"/>
		                		<xsl:with-param name="warningMessage" select="' el tipo de documento de identidad del transportista no es valido (06 - RUC).'"/>
		                    </xsl:call-template>
		                </xsl:if>

	                	<xsl:if test="./cac:PartyIdentification/cbc:ID/@schemeID = '6' and not(regexp:match(./cac:PartyIdentification/cbc:ID,'^[0-9]{11}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4163'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del transportista no cumple con un formato para RUC. '"/>
                    		</xsl:call-template>
	                	</xsl:if>
                ...
                <xsl:if test="not(./cac:PartyName/cbc:Name)">
                    <xsl:call-template name="addWarning">
                		...
                		<xsl:with-param name="warningMessage" select="' Ingrese nombres y apeelidos, o razon social del transportista de la informacion que sustento de traslado.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test='./cac:PartyName/cbc:Name and not(regexp:match(./cac:PartyName/cbc:Name,"^(?\s*$).{3,100}$"))'>
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4165'"/>
                		<xsl:with-param name="warningMessage" select="' Nombres y apeelidos, o razon social del transportista no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>

	        </xsl:for-each>

	        <!--  Valida el vehiculo principal -->
	        <xsl:if test='./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID and not(regexp:match(./sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID,"^[A-Za-z0-9 ]{8}$"))'>
	        	<xsl:call-template name="addWarning">
                	<xsl:with-param name="warningCode" select="'4167'"/>
           			<xsl:with-param name="warningMessage" select="' Nro de placa del vehiculo en informacion que sustenta el traslado no cumple con un formato valido.'"/>
            	</xsl:call-template>
	        </xsl:if>

	        <xsl:for-each select="./sac:Shipment/cac:TransportHandlingUnit">
		        <xsl:if test='./cbc:ID = $SUNATEmbededDespatchAdvice/sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID'>
		        	<xsl:call-template name="addWarning">
	                	<xsl:with-param name="warningCode" select="'4166'"/>
	           			<xsl:with-param name="warningMessage" select="' los tags donde se consigna el numero de placa del vehiculo principal no coinciden .'"/>
	            	</xsl:call-template>
		        </xsl:if>

		        <xsl:if test='./cbc:ID = $SUNATEmbededDespatchAdvice/sac:Shipment/sac:ShipmentStage/cac:TransportMeans/cac:RoadTransport/cbc:LicensePlateID'>
		        	<xsl:if test='./cbc:ID and not(regexp:match(./cbc:ID,"^[A-Za-z0-9 ]{6,8}$"))'>
			        	<xsl:call-template name="addWarning">
		                	<xsl:with-param name="warningCode" select="'4168'"/>
		           			<xsl:with-param name="warningMessage" select="' Nro de placa del vehiculo principal en informacion que sustenta el traslado no cumple con un formato valido.'"/>
		            	</xsl:call-template>
		        	</xsl:if>

		        	<xsl:if test='not(./cac:TransportEquipment/cbc:ID)'>
			        	<xsl:call-template name="addWarning">
		                	<xsl:with-param name="warningCode" select="'4169'"/>
		           			<xsl:with-param name="warningMessage" select="' Ingrese al menos un vehiculo secundario en informacion que sustenta el traslado.'"/>
		            	</xsl:call-template>
		        	</xsl:if>

		        	<xsl:for-each select="./cac:TransportEquipment">
		        		<xsl:if test='not(./cbc:ID) or not(regexp:match(./cbc:ID,"^[A-Za-z0-9 ]{6,8}$"))'>
		        			<xsl:call-template name="addWarning">
		                		<xsl:with-param name="warningCode" select="'4170'"/>
		           				<xsl:with-param name="warningMessage" select="' Nro de placa del vehiculo secundario en informacion que sustenta el traslado no cumple con un formato valido.'"/>
		            		</xsl:call-template>
		            	</xsl:if>
		        	</xsl:for-each>
		        </xsl:if>
	        ...
	        <xsl:for-each select="/sac:Shipment/sac:ShipmentStage/sac:DriverPerson">

            	<!-- Debe existir el tag de numero de documento de identidad del destinatario.
					De no cumplir generar código de error 4147 -->
                <xsl:if test="not(./cbc:ID)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4171'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese el numero de documento de identidad del conductor de la informacion que sustento de traslado.'"/>
                    </xsl:call-template>
                </xsl:if>

            	<xsl:if test="./cbc:CustomerAssignedAccountID">

                    <xsl:if test="not(./cbc:ID/@schemeID)">
	                    <xsl:call-template name="addWarning">
		                    <xsl:with-param name="warningCode" select="'4172'"/>
	                		<xsl:with-param name="warningMessage" select="' Para informacion que sustenta el traslado de bienes, debe Ingresar el tipo de documento de identidad del conductor'"/>
	                    </xsl:call-template>
	                </xsl:if>

	                <xsl:if test="./cbc:ID/@schemeID">

		                <xsl:if test="./cbc:ID/@schemeID != '0' and ./cbc:ID/@schemeID != '1' and ./cbc:ID/@schemeID != '4' and ./cbc:ID/@schemeID != '7' and ./cbc:ID/@schemeID != 'A'">
		                    <xsl:call-template name="addWarning">
			                    <xsl:with-param name="warningCode" select="'4173'"/>
		                		<xsl:with-param name="warningMessage" select="' el tipo de documento de identidad del conductor no es valido (catalogo N° 06).'"/>
		                    </xsl:call-template>
		                </xsl:if>

	                	<xsl:if test="./cbc:ID/@schemeID = '0' and not(regexp:match(./cbc:ID,'^[A-Za-z0-9]{1,15}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="errorCode" select="'4174'"/>
	                			<xsl:with-param name="errorMessage" select="' Numero de documento de identidad del conductor no cumple con un formato valido para el tipo ingresado. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:ID/@schemeID = '1' and not(regexp:match(./cbc:ID,'^[0-9]{8}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4174'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del conductor no cumple con un formato valido para DNI. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:ID/@schemeID = '4' and not(regexp:match(./cbc:ID,'^[A-Za-z0-9]{1,12}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4174'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del conductor no cumple con un formato para Carnet de Extranjeria. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:ID/@schemeID = '7' and not(regexp:match(./cbc:ID,'^[A-Za-z0-9]{1,12}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4174'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del conductor no cumple con un formato para PASAPORTE. '"/>
                    		</xsl:call-template>
	                	</xsl:if>

	                	<xsl:if test="./cbc:ID/@schemeID = 'A' and not(regexp:match(./cbc:ID,'^[A-Za-z0-9]{15}$'))">
	                    	<xsl:call-template name="addWarning">
		                    	<xsl:with-param name="warningCode" select="'4174'"/>
	                			<xsl:with-param name="warningMessage" select="' Numero de documento de identidad del conductor no cumple con un formato para el tipo ingresado. '"/>
                    		</xsl:call-template>
	                	</xsl:if>
	        ...
	        <xsl:for-each select="./sac:Shipment/cac:Delivery/cac:DeliveryAddress">

            	<!-- Debe existir el tag codigo de ubigeo del punto de llegada.
					De no cumplir generar código de error 4175 -->
                <xsl:if test="not(./cbc:ID)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4175'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese codigo de ubigeo del punto de llegada en informacion que sustenta el traslado de bienes.'"/>
                    </xsl:call-template>
                </xsl:if>

                <!-- Debe existir el tag codigo de ubigeo del punto de llegada.
					De no cumplir generar código de error 4176 -->
                <xsl:if test="./cbc:ID and not(regexp:match(./cbc:ID,'^[0-9]{6}$'))">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4176'"/>
                		<xsl:with-param name="warningMessage" select="' Codigo de ubigeo del punto de llegada no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test="not(./cbc:StreetName)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4178'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese direccion completa y detallada del punto de llegada en informacion que sustenta el traslado de bienes.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test='./cbc:StreetName and not(regexp:match(./cbc:StreetName,"^(?\s*$).{3,100}$"))'>
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4179'"/>
                		<xsl:with-param name="warningMessage" select="' Direccion completa y detallada del punto de llegada no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>

	        </xsl:for-each>

	        <xsl:for-each select="./sac:Shipment/cac:OriginAddress">

            	<!-- Debe existir el tag codigo de ubigeo del punto de Partida.
					De no cumplir generar código de error 4180 -->
                <xsl:if test="not(./cbc:ID)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4180'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese codigo de ubigeo del punto de partida en informacion que sustenta el traslado de bienes.'"/>
                    </xsl:call-template>
                </xsl:if>

                <!-- Debe existir el tag codigo de ubigeo del punto de llegada.
					De no cumplir generar código de error 4181 -->
                <xsl:if test="./cbc:ID and not(regexp:match(./cbc:ID,'^[0-9]{6}$'))">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4181'"/>
                		<xsl:with-param name="warningMessage" select="' Codigo de ubigeo del punto de partida no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test="not(./cbc:StreetName)">
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4183'"/>
                		<xsl:with-param name="warningMessage" select="' Ingrese direccion completa y detallada del punto de partida en informacion que sustenta el traslado de bienes.'"/>
                    </xsl:call-template>
                </xsl:if>

                <xsl:if test='./cbc:StreetName and not(regexp:match(./cbc:StreetName,"^(?\s*$).{3,100}$"))'>
                    <xsl:call-template name="addWarning">
	                    <xsl:with-param name="warningCode" select="'4184'"/>
                		<xsl:with-param name="warningMessage" select="' Direccion completa y detallada del punto de partida no cumple con un formato valido.'"/>
                    </xsl:call-template>
                </xsl:if>
        ...
        <!-- INI wsandovalh Se agrega no duplicado para 2001 Percepciones (Catalogo 14) -->
        <xsl:if test="(count($additionalMonetaryTotalID[text()='1001'])>1 or
				...
				count($additionalMonetaryTotalID[text()='3001'])>1 or
				count($additionalMonetaryTotalID[text()='2001'])>1)"> <!-- wsandovalh 2001 Percepciones -->
        ...
        <!-- FIN wsandovalh Se agrega no duplicado para 2001 Percepciones (Catalogo 14) -->

        <!-- INI wsandovalh Declaracion de variables -->
		<xsl:variable name="legalMonetaryTotalPayableAmount" select="cac:LegalMonetaryTotal/cbc:PayableAmount"/>
		<xsl:variable name="documentCurrencyCode" select="cbc:DocumentCurrencyCode"/>
		<!-- FIN wsandovalh Declaracion de variables -->

        <xsl:for-each select="$additionalMonetaryTotal">

			<xsl:choose>
				<!-- INI wsandovalh Validaciones Factura-Percepcion GEM; cbc:ID = 2001 -->
				<xsl:when test="./cbc:ID[text()='2001']">

					<!-- sac:AdditionalMonetaryTotal/cbc:ID/@schemeID -->
					<xsl:choose>
						<xsl:when test="not(string(./cbc:ID/@schemeID))">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2784'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2784)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when test="not(string(./sac:ReferenceAmount)) or (./sac:ReferenceAmount[text()&lt;=0])">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2785'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2785)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="not(regexp:match(./sac:ReferenceAmount, '^[0-9]{1,12}(\.[0-9]{1,2})?$'))">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2786'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2786)'" />
								</xsl:call-template>
							</xsl:if>

							<!-- SI moneda es PEN, sac:AdditionalMonetaryTotal/sac:ReferenceAmount debe ser menor o igual a /Invoice/cac:LegalMonetaryTotal/cbc:PayableAmount -->
							<xsl:if test="$documentCurrencyCode[text() = 'PEN'] and number(./sac:ReferenceAmount) &gt; number($legalMonetaryTotalPayableAmount)">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2797'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2797)'" />
								</xsl:call-template>
							</xsl:if>
					...
					<!-- sac:AdditionalMonetaryTotal/sac:ReferenceAmount/@currencyID -->
					<xsl:choose>
						<xsl:when test="not(string(./sac:ReferenceAmount/@currencyID))">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2787'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2787)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="./sac:ReferenceAmount/@currencyID != 'PEN'">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2788'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2788)'" />
								</xsl:call-template>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="not(string(./cbc:PayableAmount)) or (./cbc:PayableAmount[text()&lt;=0])">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2789'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2789)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="not(regexp:match(./cbc:PayableAmount, '^[0-9]{1,12}(\.[0-9]{1,2})?$'))">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2790'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2790)'" />
								</xsl:call-template>
							</xsl:if>
							...
							<!-- tasa * sac:ReferenceAmount = cbc:PayableAmount -->
							<xsl:if test="(round((number($porTasa div 100) * number(./sac:ReferenceAmount)) * 100) div 100) != number(./cbc:PayableAmount)">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2798'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2798)'" />
								</xsl:call-template>
							</xsl:if>
					...
					<!-- sac:AdditionalMonetaryTotal/cbc:PayableAmount/@currencyID -->
					<xsl:choose>
						<xsl:when test="not(string(./cbc:PayableAmount/@currencyID))">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2791'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2791)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="./cbc:PayableAmount/@currencyID != 'PEN'">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2792'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2792)'" />
								</xsl:call-template>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="not(string(./sac:TotalAmount)) or (./sac:TotalAmount[text()&lt;=0])">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2793'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2793)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="not(regexp:match(./sac:TotalAmount, '^[0-9]{1,12}(\.[0-9]{1,2})?$'))">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2794'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2794)'" />
								</xsl:call-template>
							</xsl:if>

							<!-- sac:ReferenceAmount + cbc:PayableAmount = sac:TotalAmount -->
							<xsl:if test="(((number(./sac:ReferenceAmount) + number(./cbc:PayableAmount)) * 100) div 100) != number(./sac:TotalAmount)">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2799'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2799)'" />
								</xsl:call-template>
							</xsl:if>
					...
					<!-- sac:AdditionalMonetaryTotal/sac:TotalAmount/@currencyID -->
					<xsl:choose>
						<xsl:when test="not(string(./sac:TotalAmount/@currencyID))">
							<xsl:call-template name="rejectCall">
								<xsl:with-param name="errorCode" select="'2795'" />
								<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2795)'" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="./sac:TotalAmount/@currencyID != 'PEN'">
								<xsl:call-template name="rejectCall">
									<xsl:with-param name="errorCode" select="'2796'" />
									<xsl:with-param name="errorMessage" select="'Error Expr Regular Factura (codigo: 2796)'" />
								</xsl:call-template>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				...
				<!-- FIN wsandovalh Validaciones Factura-Percepcion GEM; cbc:ID = 2001 -->
				<xsl:otherwise>
